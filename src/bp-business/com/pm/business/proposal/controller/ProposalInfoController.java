package com.pm.business.proposal.controller;
...
import com.pm.common.print.ProposalInfoPrint;
import com.pm.common.print.ProposalPrint;
...
@Controller
@RequestMapping("/ProposalInfoController")
public class ProposalInfoController {
...
//			@FormModel ("repairOrderInfo")RepairOrderInfoDTO repairOrderInfo,@FormModel ("pageflag")String pageflag,@FormModel ("repair_user_flag")String repair_user_flag,
//			@FormModel Processor processor) {
//		JSONMap<String, Object> model = new JSONMap<String, Object>();
//		try {
//			String pageflag2 = (pageflag==null)?"":pageflag;
//			String repair_user_flag2 = (repair_user_flag==null)?"":repair_user_flag;
//			Pagination<RepairOrderInfoDTO> repairOrderInfoList = repairOrderInfoService.queryRepairOrderInfoList(repairOrderInfo,pageflag2,repair_user_flag2,processor.getPageIndex(), processor.getPageSize(),processor);
				...
				if(repairResultEntity != null){
					...
					if(cnt>0){
						...
						if(sysUserInfoList.size()>0){
							...
							mu.setEmailUserName(sysUserInfoList.get(0).getUser_nm());
			...
			if(entity!=null){
				...
				if(offerInfoList.size()>0){
					...
					if(factoryManagerInfo.size()>0){
						...
						mu.setEmailUserName(factoryManagerInfo.get(0).getUser_nm());
...
//			String outPutUrl = repairPrint.mdPrint(map审批流, map维修单, list报价, list维修结果,list前期报价);
	...
	private List GetReapirOffer(String repair_result_id){
		...
		if(!"".equals(repair_result_id) && repair_result_id !=null){
			...
			for(int i = 0; i < repairCostDetailList.size(); i++){
				...
				map报价内容.put("描述", repairCostDetail.getRemark());
	...
	private List GetBeforeReapirOffer(String offer_id){
		...
		if(!"".equals(offer_id) && offer_id !=null){
			...
			for(int i = 0; i < offerDetailList.size(); i++){
				...
				map前期报价.put("报价描述", entity.getRemark());
	...
	private List GetItemAccountResult(List<RepairOrderInfo> list,HttpServletRequest request){
		...
		if(list.size()>0){
			...
			for(int i = 0; i < list.size(); i++){
				...
				repairCostDetail.setRepair_result_id(repairResultInfo.getRepair_result_id());
				List<RepairCostDetail> repairCostList= repairOrderInfoService.queryRepairCostDetailList(repairCostDetail);
				if(repairCostList.size()>0){
//					for(int j = 0; j < repairCostList.size(); j++){
//						repairCostDetail = repairCostList.get(j);
//						if(!PmConstant.PERSON_EXPENSES_TYPE.equals(repairCostDetail.getCost_type())){
...
//					}
					...
					allcount = allcount + printbase.tRound(OfferInfo.getTax());
					if(allcount != 0){
						Map map维修材料费 = new HashMap();
...
//		}
	...
	public @ResponseBody JSONMap<String, Object> queryDownListForProposal(
		...
		JSONMap<String, Object> model = new JSONMap<String, Object>();
		try {
			sysCodeInfo.setCode_group_value(PmConstant.PROPOSAL_ITEM);
			List<SysCodeInfo> proposalItem = sysCodeInfoService.querySysCodeInfoListNoPageWithCode(sysCodeInfo);
			...
			model.put("proposalItem", proposalItem);
			model.put("sysUserInfoList", sysUserInfoList);
			model.put("factoryManagerInfo", factoryManagerInfo);
			model.put("sysUserInfo2", (sysUserInfoList2.size()>0)?sysUserInfoList2.get(0):"");
			model.put("sysUserInfo3", (sysUserInfoList3.size()>0)?sysUserInfoList3.get(0):"");
	 ...
	 * @return
	 */
	@RequestMapping("/addProposalInfoData.json")
	public @ResponseBody JSONMap<String, Object> addProposalInfoData(
			@FormModel ProposalInfo proposalInfo,@FormModel SysUserInfo sysUserInfo,
			@FormModel ("approveInfoList") List<ProposalApproveInfo> approveInfoList, @FormModel Processor processor) {
		JSONMap<String, Object> model = new JSONMap<String, Object>();
		try {
			proposalInfo.setCreate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
			proposalInfo.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
			proposalInfo.setCreate_id(processor.getCurrentUserId());
			proposalInfo.setUpdate_id(processor.getCurrentUserId());
			proposalInfo.setApproval_state(PmConstant.APPROVE_WAIT);
			proposalInfo.setApproval_step(PmConstant.APPROVE);
			String str = proposalInfoService.saveProposalInfo(proposalInfo);
			int count=1;
//			MailUtil mu = new MailUtil();
			for(ProposalApproveInfo approveInfo:approveInfoList){
				approveInfo.setProposal_num(str);
				approveInfo.setApprove_step(count+"");
				if(count == 1){
					approveInfo.setApprove_yn(PmConstant.STATE_Y);
					approveInfo.setApproval_state(PmConstant.APPROVE_STATE);
				}else if(count == 2){
					proposalInfo.setDeal_user_id(approveInfo.getApprove_user_id());
					approveInfo.setApprove_yn(PmConstant.STATE_N);
...
//					}
			...
			info.setProposal_num(str);
			List<ProposalInfo> list = proposalInfoService.queryProposalInfoList(info);
			if(list.size()>0){
				info = list.get(0);
				info.setDeal_user_id(proposalInfo.getDeal_user_id());
				proposalInfoService.updateProposalInfo(info);
			}
			//发送邮件
//			MailUtil.sendMail(mu);
			model.put(SysConstant.OP_FLAG, true);
	 ...
	 * @return
	 */
	@RequestMapping("/queryProposalInfoList.json")
	public @ResponseBody JSONMap<String, Object> queryProposalInfoList(
			@FormModel ("proposalInfo")ProposalInfoDTO proposalInfo,
			@FormModel Processor processor) {
		JSONMap<String, Object> model = new JSONMap<String, Object>();
		try {
			Pagination<ProposalInfoDTO> proposalInfoList = proposalInfoService.queryProposalInfoListBase(proposalInfo,processor.getPageIndex(), processor.getPageSize());
			List<ProposalInfoDTO> list = proposalInfoList.getRecords();
			for(ProposalInfoDTO item:list){
				if(processor.getCurrentUserId().equals(item.getDeal_user_id())){
					if(PmConstant.PROPOSAL_APPROVE_WAIT.equals(item.getApproval_state()) && PmConstant.PROPOSAL_APPROVE.equals(item.getApproval_step())){
						item.setFlag("1");//shenpi bohui
					}else if(PmConstant.PROPOSAL_DISTRIBUT_WAIT.equals(item.getApproval_state()) && PmConstant.PROPOSAL_DISTRIBUT.equals(item.getApproval_step())){
						item.setFlag("2");//fenpei
					}else if(PmConstant.PROPOSAL_CONFIRM_WAIT.equals(item.getApproval_state()) && PmConstant.PROPOSAL_CONFIRM.equals(item.getApproval_step())
							|| (PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM_WAIT.equals(item.getApproval_state()) && PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM.equals(item.getApproval_step()))){
						item.setFlag("4");//queren
					}else if(PmConstant.PROPOSAL_OPERATE_WAIT.equals(item.getApproval_state()) && PmConstant.PROPOSAL_OPERATE.equals(item.getApproval_step())){
						item.setFlag("5");//shishi
					}else{
						item.setFlag("6");
					}
				}
			...
			proposalInfoList.setRecords(list);
			UIDataConvert<ProposalInfoDTO> convert = new UIDataConvert<ProposalInfoDTO>();
			IConvert approvalStatus = new SqlMapConvertImpl("approval_state","approval_state_nm", "sys_code_info","code_nm", "code_value","and code_group_value='proposal_approve_state'");
			IConvert dealUser = new SqlMapConvertImpl("deal_user_id","deal_user_id_nm", "sys_user_info","user_nm", "user_num","");
			IConvert createUser = new SqlMapConvertImpl("create_id","create_id_nm", "sys_user_info","user_nm", "user_num","");
			IConvert repairUser = new SqlMapConvertImpl("operate_user","operate_user_nm", "sys_user_info","user_nm", "user_num","");
			IConvert userDepart = new SqlMapConvertImpl("dept_code","dept_nm", "sys_code_info","code_nm", "code_value","and code_group_value='depart_group'");
			IConvert moneyYn = new SqlMapConvertImpl("proposal_type","proposal_type_nm", "sys_code_info","code_nm", "code_value","and code_group_value='proposal_item'");
			convert.addConvert(approvalStatus);
			convert.addConvert(dealUser);
			convert.addConvert(createUser);
			convert.addConvert(repairUser);
			convert.addConvert(userDepart);
			convert.addConvert(moneyYn);
			convert.format(proposalInfoList);
			model.put("data", proposalInfoList);
			model.put(SysConstant.OP_FLAG, true);
			model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping("/deleteProposalInfoData.json")
	public @ResponseBody JSONMap<String, Object> deleteProposalInfoData(
			@FormModel ProposalInfo proposalInfo,@FormModel ProposalApproveInfo proposalApproveInfo,
			@FormModel Processor processor) {
		JSONMap<String, Object> model = new JSONMap<String, Object>();
		ProposalInfo entity = proposalInfoService.getProposalInfoById(proposalInfo.getId());
		if(entity!=null){
			proposalApproveInfo.setProposal_num(entity.getProposal_num());
			List<ProposalApproveInfo> list = proposalInfoService.queryProposalApproveInfoList(proposalApproveInfo);
			for(ProposalApproveInfo info:list){
				proposalInfoService.deleteProposalApproveInfo(info);
			}
			int cnt = proposalInfoService.deleteProposalInfo(proposalInfo);
			if (cnt > 0) {
				model.put(SysConstant.OP_FLAG, true);
				model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
			} else {
	 ...
	 * @return
	 */
	@RequestMapping("/queryProposalInfoDetail.json")
	public @ResponseBody JSONMap<String, Object> queryProposalInfoDetail(
			@FormModel ProposalInfo proposalInfo,@FormModel ProposalApproveInfo proposalApproveInfo, @FormModel SysCodeInfo sysCodeInfo,
			@FormModel SysUserInfo sysUserInfo,@FormModel Processor processor) {
		JSONMap<String, Object> model = new JSONMap<String, Object>();
		String draftUser = "";
		List<ProposalInfo> proposalInfoList = proposalInfoService.queryProposalInfoList(proposalInfo);
		if(proposalInfoList.size()>0){
			proposalApproveInfo.setProposal_num(proposalInfoList.get(0).getProposal_num());
...
//						}
					...
					if(PmConstant.PROPOSAL_APPROVE.equals(item.getApprove_step())){
						model.put("appuser1", item);
					}else if(PmConstant.DISTRIBUT.equals(item.getApprove_step())){
						model.put("appuser2", item);
					}else if(PmConstant.PROPOSAL_CONFIRM.equals(item.getApprove_step())){
						model.put("appuser3", item);
					}else if(PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM.equals(item.getApprove_step())){
						model.put("appuser4", item);
					}else if(PmConstant.PROPOSAL_DRAFT.equals(item.getApprove_step())){
						draftUser = item.getApprove_user_id();
					}
					model.put("proposalInfo", proposalInfoList.get(0));
				}
			}
		}
		sysCodeInfo.setCode_group_value(PmConstant.PROPOSAL_ITEM);
	 ...
	 * @return
	 */
	@RequestMapping("/updateProposalInfoData.json")
	public @ResponseBody JSONMap<String, Object> updateProposalInfoData(
			@FormModel ProposalInfo proposalInfo,
			@FormModel ("approveInfoList") List<ProposalApproveInfo> approveInfoList, @FormModel Processor processor) {
		JSONMap<String, Object> model = new JSONMap<String, Object>();
		try {
			int count=1;
			String create_id="";
			String create_time="";
			ProposalApproveInfo entity = new ProposalApproveInfo();
			ProposalInfo proposalInfoEntity = proposalInfoService.getProposalInfoById(proposalInfo.getId());
			if(proposalInfoEntity != null){
				entity.setProposal_num(proposalInfoEntity.getProposal_num());
				List<ProposalApproveInfo> list = proposalInfoService.queryProposalApproveInfoList(entity);
				if(list.size()>0){
					create_id=list.get(0).getCreate_id();
					create_time=list.get(0).getCreate_time();
					for(ProposalApproveInfo item:list){
						proposalInfoService.deleteProposalApproveInfo(item);
					}
				}
				for(ProposalApproveInfo approveInfo:approveInfoList){
					approveInfo.setProposal_num(proposalInfoEntity.getProposal_num());
					approveInfo.setApprove_step(count+"");
					if(count == 1){
						approveInfo.setApprove_yn(PmConstant.STATE_Y);
						approveInfo.setApproval_state(PmConstant.APPROVE_STATE);
					...
					proposalInfo.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
					proposalInfo.setUpdate_id(processor.getCurrentUserId());
					int cnt = proposalInfoService.updateProposalInfo(proposalInfo);
					if(cnt>0){
						model.put(SysConstant.OP_FLAG, true);
						model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
					}
	...
	//-----------------------------------------------以下是审批，驳回，确认管理
		...
		public @ResponseBody JSONMap<String, Object> updateProposalInfoApproveAndReject(
			...
			try {
				...
				SysUserInfo sysUserSendMailInfoList = sysUserInfoService.getSysUserInfoById(entity.getCreate_id());
				...
				if(approList.size()>0){
					...
					for(int i = 0;i<approList.size();i++){
						...
						if(PmConstant.PROPOSAL_DRAFT.equals(approList.get(i).getApprove_step())){
							draftUser = approList.get(i).getApprove_user_id();
						}
					}
					for(int i = 0;i<approList.size();i++){
...
//				MailUtil mu = new MailUtil();
				...
				entity.setDeal_user_id(tempEntity.getApprove_user_id());
				if(PmConstant.REJECT_STATE.equals(updateflag)){
					entity.setApproval_state(PmConstant.PROPOSAL_REJECT);
					entity.setDeal_user_id("");
					tempEntity.setApproval_state(PmConstant.REJECT_STATE);
					tempEntity.setApprove_user_id(tempApproUser);
					sysUserInfo.setUser_num(draftUser);
					...
					//发送驳回邮件
					String htmlSubject = "提案审批驳回";
					String htmlContent = "<p><span style='font-size: 18px;'>您好：<strong>"+draftUser+"</strong></span></p>"+
							
							 "<p>&nbsp; &nbsp; &nbsp; &nbsp;您的提案编号为："+entity.getProposal_num()+"的提案被驳回，请查看！</p>";
		
					//sendMailbyUserInfo(htmlContent,htmlSubject,sysUserSendMailInfoList.getEmail());
					
					
//					mu.setContent(PmConstant.MAIL_REJECT);
				}else if(PmConstant.APPROVE_STATE.equals(updateflag)){
					if(PmConstant.PROPOSAL_APPROVE_WAIT.equals(entity.getApproval_state())){
						entity.setApproval_state(PmConstant.PROPOSAL_DISTRIBUT_WAIT);
						entity.setApproval_step(PmConstant.PROPOSAL_DISTRIBUT);
						entity.setApproval_date(DateUtil.getNow(DateUtil.Y_M_D));
					}
//					else if(PmConstant.FACTORY_MANAGER_APPROVE_WAIT.equals(entity.getApproval_state())){
//						entity.setFactory_manager_approval_date(DateUtil.getNow(DateUtil.Y_M_D));
...
//					}
					...
					//发送审批邮件
					String htmlSubject = "提案审批通过";
					String htmlContent = "<p><span style='font-size: 18px;'>您好：<strong>"+draftUser+"</strong></span></p>"+
							
							 "<p>&nbsp; &nbsp; &nbsp; &nbsp;您的提案编号为："+entity.getProposal_num()+"的提案审批通过，请查看！</p>";
...
//					mu.setContent(PmConstant.MAIL_DEAL);
					...
					if(PmConstant.PROPOSAL_CONFIRM_WAIT.equals(entity.getApproval_state())){
						entity.setApproval_state(PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM_WAIT);
						entity.setApproval_step(PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM);
						entity.setConfirm_date(DateUtil.getNow(DateUtil.Y_M_D));
						...
						if(entity != null){
							proposalOperateInfo.setProposal_num(entity.getProposal_num());
...
//						mu.setContent(PmConstant.MAIL_CONFIRM);
					}else if(PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM_WAIT.equals(entity.getApproval_state())){
						entity.setApproval_state(PmConstant.PROPOSAL_DONE);
						entity.setDeal_user_id("");
						entity.setFactory_manager_approval_date(DateUtil.getNow(DateUtil.Y_M_D));
						sendMailFlag = false;
						...
						//发送审批邮件
						String htmlSubject = "提案已确认";
						String htmlContent = "<p><span style='font-size: 18px;'>您好：<strong>"+draftUser+"</strong></span></p>"+
								
								 "<p>&nbsp; &nbsp; &nbsp; &nbsp;您的提案编号为："+entity.getProposal_num()+"的提案已确认完成，请查看！</p>";
				...
				entity.setUpdate_id(processor.getCurrentUserId());
				entity.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
				int cnt = proposalInfoService.updateProposalInfo(entity);
				if(cnt>0){
					List<ProposalApproveInfo> list = proposalInfoService.queryProposalApproveInfoList(proposalApproveInfo);
					if(list.size()>0){
...
//							}
		 ...
		 * @return
		 */
		@RequestMapping("/addProposalDistributionInfoData.json")
		public @ResponseBody JSONMap<String, Object> addProposalDistributionInfoData(
				@FormModel ProposalDistributionInfo proposalDistributionInfo,@FormModel ProposalInfo proposalInfo,@FormModel ProposalApproveInfo proposalApproveInfo,
				@FormModel Processor processor) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			try {
				ProposalInfo entity = proposalInfoService.getProposalInfoById(proposalInfo.getId());
				if(entity != null){
					proposalDistributionInfo.setProposal_num(entity.getProposal_num());
					List<ProposalDistributionInfo> distributList = proposalInfoService.queryProposalDistributionInfoList(proposalDistributionInfo);
					if(distributList.size()>0){
						model.put(SysConstant.OP_FLAG, false);
						model.put(SysConstant.OP_MESSAGE, "已存在分配记录");
					}else{
						proposalDistributionInfo.setCreate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
						proposalDistributionInfo.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
						proposalDistributionInfo.setCreate_id(processor.getCurrentUserId());
						proposalDistributionInfo.setUpdate_id(processor.getCurrentUserId());
						int cnt = proposalInfoService.saveProposalDistributionInfo(proposalDistributionInfo);
						if(cnt>0){
//							if(PmConstant.EXPENSES_N.equals(distributionInfo.getRepair_type())){
//								entity.setApproval_state(PmConstant.REPAIR_WAIT);
//								entity.setApproval_step(PmConstant.REAPIR);
...
//								entity.setApproval_step(PmConstant.FACTORY_MANAGER_APPROVE);
//							}
							entity.setApproval_state(PmConstant.PROPOSAL_OPERATE_WAIT);
							entity.setApproval_step(PmConstant.PROPOSAL_OPERATE);
							entity.setDistribute_date(DateUtil.getNow(DateUtil.Y_M_D));
							entity.setOperate_user(proposalDistributionInfo.getUser_id());
							entity.setDeal_user_id(proposalDistributionInfo.getUser_id());
							entity.setUpdate_id(processor.getCurrentUserId());
							entity.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
							int count = proposalInfoService.updateProposalInfo(entity);
							if(count>0){
								proposalApproveInfo.setProposal_num(entity.getProposal_num());
								proposalApproveInfo.setApprove_step(PmConstant.PROPOSAL_DISTRIBUT);
								List<ProposalApproveInfo> approList = proposalInfoService.queryProposalApproveInfoList(proposalApproveInfo);
								if(approList.size()>0){
									ProposalApproveInfo info = approList.get(0);
									info.setApprove_yn(PmConstant.STATE_Y);
									info.setApproval_state(PmConstant.APPROVE_STATE);
									proposalInfoService.updateProposalApproveInfo(proposalApproveInfo);
								}
							}
//							MailUtil mu = new MailUtil();
//							SysUserInfo sysUserInfo = new SysUserInfo();
...
//							}
		 ...
		 * @return
		 */
		@RequestMapping("/getProposalDetailNm.json")
		public @ResponseBody JSONMap<String, Object> getProposalDetailNm(
				@FormModel ProposalInfo proposalInfo,@FormModel ProposalDistributionInfo proposalDistributionInfo,
				@FormModel Processor processor) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			ProposalInfo entity = proposalInfoService.getProposalInfoById(proposalInfo.getId());
			if(entity != null){
				...
				List<ProposalDistributionInfo> distributList = proposalInfoService.queryProposalDistributionInfoList(proposalDistributionInfo);
				if(distributList.size()>0){
					model.put("proposalDistributionInfoEntity", distributList.get(0));
				}
				model.put("proposalEntity", entity);
				model.put("proposalDetailNm", entity.getProposal_detail());
				model.put(SysConstant.OP_FLAG, true);
				model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
			}else{
				model.put(SysConstant.OP_FLAG, false);
		 ...
		 * @return
		 */
		@RequestMapping("/addProposalResultData.json")
		public @ResponseBody JSONMap<String, Object> addProposalResultData(
				@FormModel ProposalOperateInfo proposalOperateInfo,@FormModel ProposalInfo proposalInfo,@FormModel ProposalApproveInfo proposalApproveInfo,
				@FormModel Processor processor) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			try {
				ProposalInfo entity = proposalInfoService.getProposalInfoById(proposalInfo.getId());
				if(entity != null){
					proposalOperateInfo.setProposal_num(entity.getProposal_num());
					List<ProposalOperateInfo> repairResultList = proposalInfoService.queryProposalOperateInfoList(proposalOperateInfo);
					if(repairResultList.size()>0){
						model.put(SysConstant.OP_FLAG, false);
						model.put(SysConstant.OP_MESSAGE, "已存在实施记录");
					}else{
						proposalOperateInfo.setCreate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
						proposalOperateInfo.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
						proposalOperateInfo.setCreate_id(processor.getCurrentUserId());
						proposalOperateInfo.setUpdate_id(processor.getCurrentUserId());
						int count = proposalInfoService.saveProposalOperateInfo(proposalOperateInfo);
						if(count>0){
							entity.setApproval_state(PmConstant.PROPOSAL_CONFIRM_WAIT);
							entity.setDeal_date(DateUtil.getNow(DateUtil.Y_M_D));
							entity.setApproval_step(PmConstant.PROPOSAL_CONFIRM);
							entity.setUpdate_id(processor.getCurrentUserId());
							entity.setUpdate_time(DateUtil.getNow(DateUtil.Y_M_D_HMS));
							proposalApproveInfo.setProposal_num(entity.getProposal_num());
							proposalApproveInfo.setApprove_step(PmConstant.PROPOSAL_CONFIRM);
							List<ProposalApproveInfo> approList = proposalInfoService.queryProposalApproveInfoList(proposalApproveInfo);
//							SysUserInfo sysUserInfo = new SysUserInfo();
							if(approList.size()>0){
								entity.setDeal_user_id(approList.get(0).getApprove_user_id());
//								sysUserInfo.setUser_num(approList.get(0).getApprove_user_id());
							...
							proposalInfoService.updateProposalInfo(entity);
						}
//						if(cnt>0){
//							MailUtil mu = new MailUtil();
//							List<SysUserInfo> sysUserInfoList = sysUserInfoService.querySysUserInfoList(sysUserInfo);
...
//						}
		 ...
		 * @return
		 */
		@RequestMapping("/getProposalResultDetail.json")
		public @ResponseBody JSONMap<String, Object> getProposalResultDetail(
				@FormModel ProposalInfo proposalInfo,@FormModel ProposalOperateInfo proposalOperateInfo,@FormModel Processor processor) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			ProposalInfo entity = proposalInfoService.getProposalInfoById(proposalInfo.getId());
			if(entity != null){
				proposalOperateInfo.setProposal_num(entity.getProposal_num());
				List<ProposalOperateInfo> proposalOperateList = proposalInfoService.queryProposalOperateInfoList(proposalOperateInfo);
				if(proposalOperateList.size()>0){
					model.put("proposalOperateEntity", proposalOperateList.get(0));
					model.put(SysConstant.OP_FLAG, true);
					model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
				}
			}else{
		 ...
		 * @return
		 */
		@RequestMapping("/updateProposalImprovementEffect.json")
		public @ResponseBody JSONMap<String, Object> updateProposalImprovementEffect(
				@FormModel ProposalInfo proposalInfo,@FormModel ProposalOperateInfo proposalOperateInfo,@FormModel Processor processor) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			String improvement_effect ="";
			if(proposalOperateInfo != null){
			...
			if(entity != null){
				...
				if(proposalOperateList.size()>0){
					proposalOperateInfo = proposalOperateList.get(0);
					proposalOperateInfo.setImprovement_effect(improvement_effect);
					int cnt = proposalInfoService.updateProposalOperateInfo(proposalOperateInfo);
					if(cnt>0){
						model.put(SysConstant.OP_FLAG, true);
						model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
					}
		 ...
		 * @return
		 */
		@RequestMapping("/proposalExport.json")
		public @ResponseBody JSONMap<String, Object> proposalExport(
				@FormModel ProposalInfo proposalInfo, @FormModel Processor processor,HttpServletRequest request) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			try{
				
				//审批流信息取得
				Map map审批流 = this.GetApproveline2(proposalInfo);
				//提案书信息取得
				Map map提案书 = this.GetProposalInfo(proposalInfo);
				//提案书实施结果信息取得
				Map map实施结果 = this.GetProposalResultInfo(proposalInfo);
				
				ProposalPrint proposalPrint = new ProposalPrint(request);
				String outPutUrl = proposalPrint.mdPrint(map审批流, map提案书, map实施结果);
		 ...
		 * @return
		 */
		private Map<String,String> GetApproveline2(ProposalInfo entity){
			//审批流信息取得
			List<ProposalInfoDTO> roposalInfoList = proposalInfoService.queryProposalInfoList2(entity);
			Map map审批流 = new HashMap();
			if(roposalInfoList.size() > 0){
				ProposalInfoDTO roposalInfo = roposalInfoList.get(0);
				//起草
				String lcCreate_time = roposalInfo.getCreate_time().substring(0,10);
				//初审
				String lcApprove_date = roposalInfo.getApproval_date();
				//分配
				String lcDistribute_date = roposalInfo.getDistribute_date();
				//实施
				String lcOperate_date = roposalInfo.getDeal_date();
				//验证
				String lcConfirm_date = roposalInfo.getConfirm_date();
				//Approval
				String lcFactory_manager_confirm_date = roposalInfo.getFactory_manager_approval_date();

				//报价and维修人
				String operateName = roposalInfo.getOperate_user_nm();
				map审批流.put("实施time", lcOperate_date);
				map审批流.put("实施", operateName);
				
				ProposalApproveInfo approveInfo = new ProposalApproveInfo();
				approveInfo.setProposal_num(roposalInfo.getProposal_num());
				List<ProposalApproveInfo> approveList = proposalInfoService.queryProposalApproveInfoList(approveInfo);

				for(int i=0; i < approveList.size(); i++){
					approveInfo = approveList.get(i);
					String lcApprove_user_nm = approveInfo.getApprove_user_nm();
					//申请
					if(PmConstant.PROPOSAL_DRAFT.equals(approveInfo.getApprove_step())){
						map审批流.put("申请", lcApprove_user_nm);
						map审批流.put("申请time", lcCreate_time);
					}
					//初审
					else if(PmConstant.PROPOSAL_APPROVE.equals(approveInfo.getApprove_step())){
						map审批流.put("初审", lcApprove_user_nm);
						map审批流.put("初审time", lcApprove_date);
					}
					//分配
					else if(PmConstant.PROPOSAL_DISTRIBUT.equals(approveInfo.getApprove_step())){
						map审批流.put("分配", lcApprove_user_nm);
						map审批流.put("分配time", lcDistribute_date);
					}
					//验证
					else if(PmConstant.PROPOSAL_CONFIRM.equals(approveInfo.getApprove_step())){
						map审批流.put("验证", lcApprove_user_nm);
						map审批流.put("验证time", lcConfirm_date);
					}
					//Approval
					else if(PmConstant.PROPOSAL_FACTORY_MANAGER_CONFIRM.equals(approveInfo.getApprove_step())){
						map审批流.put("Approval", lcApprove_user_nm);
						map审批流.put("Approvaltime", lcFactory_manager_confirm_date);
					}
				}
		 ...
		 * @return
		 */
		private Map<String,String> GetProposalInfo(ProposalInfo proposalInfo){
			Map map提案书 = new HashMap();
			if(proposalInfo != null){
				List<ProposalInfoDTO> roposalInfoList = proposalInfoService.queryProposalInfoList2(proposalInfo);
				if(roposalInfoList.size()>0){
					ProposalInfoDTO entity = roposalInfoList.get(0);
					map提案书.put("申请编号", entity.getProposal_num());
					map提案书.put("提案内容", entity.getProposal_detail());
					map提案书.put("效果预期", entity.getAnticipate_result());
					map提案书.put("费用人力投入", entity.getResource_input());
					map提案书.put("比较结论", entity.getComparative_conclusion());
					map提案书.put("内容分类", entity.getProposal_type_nm());
				}
			}
			return map提案书;
		}
		 ...
		 * @return
		 */
		private Map<String,String> GetProposalResultInfo(ProposalInfo proposalInfo){
			Map map实施结果 = new HashMap();
			if(proposalInfo != null){
				List<ProposalInfoDTO> roposalInfoList = proposalInfoService.queryProposalInfoList2(proposalInfo);
				if(roposalInfoList.size()>0){
					ProposalInfoDTO proposalInfoDTO = roposalInfoList.get(0);
					ProposalOperateInfo entity = new ProposalOperateInfo();
					entity.setProposal_num(proposalInfoDTO.getProposal_num());
					List<ProposalOperateInfo> list = proposalInfoService.queryProposalOperateInfoList(entity);
					if(list.size()>0){
						entity = list.get(0);
						map实施结果.put("实施内容", entity.getOperate_detail());
						map实施结果.put("实施费用", entity.getOperate_expense());
						map实施结果.put("改善效果", entity.getImprovement_effect());
						map实施结果.put("意见", entity.getDirector_opinion());
					}
				}
			}
			return map实施结果;
		 ...
		 * @return
		 */
		@RequestMapping("/getProposalResultInfoById.json")
		public @ResponseBody JSONMap<String, Object> getProposalResultInfoById(
				@FormModel ProposalOperateInfo proposalOperateInfo,@FormModel ("id")String id,
				@FormModel Processor processor) {
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			ProposalInfo proposalInfo = proposalInfoService.getProposalInfoById(id);
			if(proposalInfo!=null){
				proposalOperateInfo.setProposal_num(proposalInfo.getProposal_num());
				List<ProposalOperateInfo> list = proposalInfoService.queryProposalOperateInfoList(proposalOperateInfo);
				if(list.size()>0){
					model.put("proposalOperateInfo", list.get(0));
					model.put(SysConstant.OP_FLAG, true);
					model.put(SysConstant.OP_MESSAGE, SysConstant.SUCCESS);
				}else{
					model.put(SysConstant.OP_FLAG, false);
		...
		public @ResponseBody JSONMap<String,Object> excelDownload(@FormModel("excelList") String[] excelList,@FormModel Processor processor,HttpServletRequest request) {
			...
			try{
				...
				List<ProposalInfoDTO> proposalInfoList = proposalInfoService.excelDownload(sourceStrArray);
				List<ProposalInfoDTO> dataList = new ArrayList<ProposalInfoDTO>();
				ProposalInfoDTO dataEntity = null;
				for(int i=0;i<proposalInfoList.size();i++){
					proposalInfoList.get(i).setNumber(""+(i+1));
				}
				ProposalInfoPrint proposalInfoPrint = new ProposalInfoPrint(request);
				String outPutUrl = proposalInfoPrint.mdPrint(proposalInfoList);
		 ...
		 * @return
		 */
		@RequestMapping("/sendMailbyUserInfo.json")
		public @ResponseBody JSONMap<String, Object>  sendMailbyUserInfo(){
			JSONMap<String, Object> model = new JSONMap<String, Object>();
			try{
				 Properties prop = new Properties();
				 prop.setProperty("mail.transport.protocol", "smtp");// 连接协议
				 prop.setProperty("mail.debug", "true");
				 prop.setProperty("mail.host", "61.161.213.5");
				 prop.setProperty("mail.smtp.port", "587");// 端口号
				 ...
				 message.setRecipient(Message.RecipientType.TO, new InternetAddress("18246103@qq.com")); // 邮件的收件人
		
				 message.setSubject("提案提醒测试"); // 邮件的标题
				 
				 MimeBodyPart text = new MimeBodyPart();
				 text.setContent("test", "text/html;charset=UTF-8");
