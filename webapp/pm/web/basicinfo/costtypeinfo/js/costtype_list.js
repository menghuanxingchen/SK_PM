...
function queryDataList(){
	...
	var parameterData = {
			url:getRequestUrl("/CostTypeController/queryDataList.json"),
...
function addEntityDataFun(){
	var page = "/pm/web/basicinfo/costtypeinfo/costtype_add.jsp";
	pageForward(page);
}
...
function updateEntityDataFun(id){
	var page = "/pm/web/basicinfo/costtypeinfo/costtype_update.jsp?id="+id;
	pageForward(page);
}
...
function deleteDataFun(id){
	...
	layer.confirm(
			...
			function(index){
				$.ajax({
					url:getRequestUrl("/CostTypeController/deleteEntityData.json"),
					dataType:"json",
