...
function initPage(){
	$.ajax({
		...
		success:function(result){
			...
			$("#repairCategory").html($("#repairCategoryDownListTemplate").render(result));
			$("#repairCategory option[value='"+Class.getConstant('EXPENSES_Y')+"']").attr("selected","selected");
			$("#stateYn").html($("#stateYndownListTemplate").render(result));
	...
	if(checkfalg){
		...
		layer.confirm(
				...
				function(index){
					...
					$.ajax({
						url:getRequestUrl("/RepairOrderInfoController/addDistributionInfoData.json"),
						dataType:"json",
						data:postData,
						success:function(result){
							layer.close(index);
