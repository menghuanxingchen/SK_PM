...
var approval_state = getRequestParameterValue("approval_state");
...
function queryRepairDataEntity(){
	$.ajax({
		url:getRequestUrl("/ProposalInfoController/getProposalResultInfoById.json"),
		dataType:"json",
		data:{"id":id},
		success:function(result){
			if(result.opflag){
...
function gotoRepairDataEntity(){
	var page = "/pm/web/proposal/proposal_result_detail.jsp?id="+id+"&&menuid="+menuid;
	pageForward(page);
}
...
function reportExport(){
	layer.load();
	$.ajax({
		url:getRequestUrl("/ProposalInfoController/proposalExport.json"),
		dataType:"json",
		data:{"proposalInfo.id":id},
		success:function(result){
			layer.closeAll('loading');
