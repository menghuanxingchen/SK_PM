;(function(window,undefined,$){
	...
	function __Init__(options){
		...
		if (options.HXGDT && options.TabStyle == "") {
		    html.push("<div style='width:2200px'>");//height:530px;
		}
		html.push("<table id='", options.elid, "_detail", "'><thead>");

		html.push("<tr class='thead'>");
		if (options.showcheckbox) {
		    html.push("<td  class='checktd'><div style='width:25px;text-align:center'><input type='checkbox' style='width:17px;height:17px' id='", options.elid, "_checkall'/></div></td>");
		}
		...
		for (var t = 0, l = k; t < l; t++) {
		    ...
		    html.push("<td style='width:" , col.width?col.width:"auto" , "'", col.sortable ? " class='sortable'" : "", "><div", style.join(""), ">", col.display, "</div></td>");
		}
		html.push("</tr>");
		...
		html.push("</div><div class='xjgrid-load'>正在加载...</div>");
		html.push("</div>");
		//
		if(options.usepager){
			//分页的代码
	...
	function __BuildRowHtml__(data, options)
	{
	    ...
	    for (var i = 0, l = data.length; i < l && i < options.rp; i++) {
	        ...
	        if (options.showcheckbox) {
	            html.push("<td class='checktd checktd_cl'><div style='width:25px;text-align:center'><input type='checkbox' style='width:17px;height:17px' id='", options.elid, "_cb_" + rowid + "' value='", data[i].id, "' ", data[i].check_state == 1 ? "checked='checked'" : "", "/></div></td>");
	        }
	        for (var j = 0 ; j < k ; j++) {
