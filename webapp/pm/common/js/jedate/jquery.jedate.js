...
;(function(window, factory) {
    ...
    jet.extend(jeDatePick.prototype,{
        ...
        dateTemplate : function() {
            ...
            var lyPrev = '<em class="yearprev yprev" @on="yearBtn(lprev,'+aowArr[0]+')"></em>',
                lyNext = '<em class="yearnext ynext" on="yearBtn(lnext,'+aowArr[2]+')"></em>',
                ryPrev = '<em class="yearprev yprev" on="yearBtn(rprev,'+aowArr[3]+')"></em>',
                ryNext = '<em class="yearnext ynext" @on="yearBtn(rnext,'+aowArr[1]+')"></em>',
                mPrev = '{% if(dlen>2){ %}<em class="monthprev mprev" @on="monthBtn(mprev,{%=daytit[i].YYYY%}-{%=daytit[i].MM%})"></em>{% } %}',
                mNext = '{% if(dlen>2){ %}<em class="monthnext mnext" @on="monthBtn(mnext,{%=daytit[i].YYYY%}-{%=daytit[i].MM%})"></em>{% } %}';
            //循环年的模板
